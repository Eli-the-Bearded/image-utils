...
For a floating point pixel value and max 1 to 65535, convert to the hexidecimal
pixel format. No error is returned
if $max is outside of the allowed range, but 0 will kill the program.
Any value larger than max is clipped.
...
sub encodepixels {
  ...
  if($$i{type} == 6) {
  	return int_encodepixels_6($p_r, $$i{deep}, $$i{encode}, $max);
  }
  if($$i{type} == 5) {
  	return int_encodepixels_5($p_r, $$i{deep}, $$i{encode}, $max);
  }
  if($$i{type} == 4) {
  	return int_encodepixels_4($p_r, $$i{deep}, $$i{encode}      );
  }
  if($$i{type} == 3) {
  	return int_encodepixels_3($p_r, $$i{deep}, $$i{encode}, $max);
  }
  if($$i{type} == 2) {
  	return int_encodepixels_2($p_r, $$i{deep}, $$i{encode}, $max);
  }
  if($$i{type} == 1) {
  	return int_encodepixels_1($p_r, $$i{deep}, $$i{encode}      );
  }
